...
def game_over() :
    ...
    level_surface = my_font.render('Your Level is :' + str(level),True,green)
    ...
    level_rect.midtop = (window_x / 2, window_y / 4+50)
...
def new_level(choice, color, font,size):
    newlevel_font = pygame.font.SysFont(font, size)
    newlevel_surface = newlevel_font.render('Level : ' + str(level), True, color)
    newlevel_rect = newlevel_surface.get_rect()
   
    newlevel_rect.midtop = (40, 20)

    # blit нарисует текст на экране
    game_window.blit(newlevel_surface, newlevel_rect)
# Main Function
while True :
    ...
    if snake_position[0] == fruit_position[0] and snake_position[1] == fruit_position[1] :
        ...
        snake_body.pop()
    if snake_position[0] == fruit_position[0] and snake_position[1] == fruit_position[1] and score%30==0 and score>0:
        level+=1
        snake_speed+=3
    ...
    obstracle = pygame.Rect(fruit_position[0], fruit_position[1], 10, 10)
    if(obstracle.height>0):
        obstracle.height=random.randrange(10,30)
        obstracle.width=random.randrange(10,30)
    pygame.draw.rect(game_window, white, obstracle)
    ...
    new_level(21,green,'times new roman',20)
